...
int main(int argc, char*argv[]) {
  ...
  i = (int *)allocate(0,4);  // size of an int is 4 bytes
  *i = 10;
  printf("value of int i %d\n", *i);
  
  c = (char*)allocate(0,1);  // size of char is 1 byte
  *c = 'H';
  printf("value of int c %c\n", *c);
  
  s = (char*)allocate(0,10);  // 10 characters long
  strncpy(s, "Hello!", 7);
  printf("value of int s '%s'\n", s);
  
  n = (struct node *)allocate(0, sizeof(struct node));
  n->v = (char*)allocate(0,20);
  n->count = 3;
