...
static struct Allocator *allocator = NULL;
...
double average_frag(){
    ...
    //calculates average
    size_t total_freespace = 0;
    size_t largest_freeblock=0;
    ...
    while(current !=NULL){
        ...
        Block *block = (Block *)current->data;
        total_freespace+= block->size;

        if(block->size >largest_freeblock){

            largest_freeblock = block->size;
        }

        current= current->next;
    }
    total_frag = 1.0- (double)largest_freeblock/ total_freespace;
